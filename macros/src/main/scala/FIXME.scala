...
object FIXME {
  def apply(format: String): Unit = macro fixmeMacro.impl
  def orDie(format: String): Unit = macro fixmeMacro.implOrDie
}
...
object fixmeMacro  {
  ...
  def impl(c: Context)(format: c.Expr[String]): c.Tree = {
    import c.universe._
  ...
  def implOrDie(c: Context)(format: c.Expr[String]): c.Tree = {
    import c.universe._
    val Literal(Constant(s_format: String)) = format.tree
    try {
      val curDate = new Date()
